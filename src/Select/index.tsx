...
import { IPicker, IPickerItemValue } from './Picker.interface';
import PickerItem, { itemHeight } from './PickerItem';
import './styles.css';
...
const Picker = <T extends unknown>({
  ...
  const handleOnValueChange = useCallback(
    (value: IPickerItemValue<T>) => onChange(value),
    [onChange]
  );
  ...
  const handleItemClick = (option: IPickerItemValue<T>) => {
    ...
    <div className="Picker_Container">
      <div style={{ position: 'relative', width: '100%', height }}>
        <div
          ...
          role="listbox"
          className="Picker_Item_Container"
          style={style}
          tabIndex={-1}
        ...
        >
          {options.map((option: IPickerItemValue<T>, index: number) => {
        ...
        <div className="Picker_Highlight" />
      </div>
    </div>
  );
};
